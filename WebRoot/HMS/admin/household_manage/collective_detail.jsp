...
<head>
	...
	<script>
			$().ready(function() {
				...
				$("#update_collective").click(function(){
					var param = $("[name='collective_name']").val();
					window.location.href = "add_collmember.html?collective_name="+encodeURI(param);
				});
...
<body>
		<h1>集体户口详细表：<em><c:out value="${requestScope.message }"></c:out></em></h1>
		<h1>集体户口名称:<c:out value="${requestScope.collective_name }"></c:out></h1>
		<input type="hidden" name="collective_name" value="<c:out value="${requestScope.collective_name }"></c:out>" />
		<div id="update">
				<table>
					<!--添加户口成员-->
					...
					<c:forEach var="member" items="${requestScope.members }">
						<tr>
							...
							<td>
								<a href="<%=basePath %>/servlet/householdmanage?func=deleteHouseholdMember&member_id=${member.id }">删除</a>
							</td>
						</tr>
				...
				<button type="button" id="update_collective">添加成员</button>
		</div>
		<div>
			<form action="<%=basePath %>/servlet/householdmanage" method="post" id="address">
				<input type="hidden" name="func" value="addAddress" />
				<input type="hidden" name="category" value="collective" />
				<input type="hidden" name="iden" value="<c:out value="${requestScope.collective_name }"></c:out>" />
				<table>
					<!--添加户口成员-->
					<tr>
						<td>省<em>*</em></td>
