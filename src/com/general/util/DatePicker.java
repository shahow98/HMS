...
public class DatePicker extends JPanel {
	...
	protected String[] weekdayNames = { "Sun", "Mon", "Tue", "Wed", "Thu",
			"Fri", "Sat" };
	...
	public Popup getPopup(Container c,int x,int y) {
		if (popup == null) {
			Point p = c.getLocation();
			PopupFactory factory = PopupFactory.getSharedInstance();
			popup = factory.getPopup(c, this, p.x+x, p.y+y);
		}
		return popup;
	}
	...
	public void popupShow(Container c,int x,int y) {
		getPopup(c,x,y);
		popup.show();
	}
	...
	protected JPanel createCalendar() {
		...
		// font.getSize() + 2);
		Font weekFont = new Font(font.getName(), font.getStyle(), font
				.getSize() - 2);
		title.setFont(font);
	...
	protected void parseDate(String date) {
		...
		if (parts.length == 3) {
			currentDisplayDate.set(Calendar.YEAR, expandYear(Integer
					.valueOf(parts[0])));
			currentDisplayDate.set(Calendar.MONTH,
					Integer.valueOf(parts[1]) - 1);
			currentDisplayDate.set(Calendar.DAY_OF_MONTH, Integer
					.valueOf(parts[2]));
	...
	public void changeDay(String day) {
		currentDisplayDate.set(Calendar.DAY_OF_MONTH, Integer.valueOf(day
				.trim()));
